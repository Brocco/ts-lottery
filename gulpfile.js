...
var port = 3000,
    paths = {
      ...
      dist: {
        ...
        style: './dist/styles',
        styleFile: 'app.css',
        styleLibFile: 'lib.css'
      },
      style: './app/app.styl',
      styles: './app/**/*.styl',
      styleLib: ['./bower_components/bootstrap/dist/css/bootstrap.min.css',
...
gulp.task('build-style-lib', function(){
  gulp.src(paths.styleLib)
    .pipe(plumber())
    .pipe(concat(paths.dist.styleLibFile))
    .pipe(gulp.dest(paths.dist.style));
});

gulp.task('build-style', ['build-style-lib'], function(){
